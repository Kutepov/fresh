...
function geoCountryCode()
{
    return strtolower($_SERVER['GEOIP_COUNTRY_CODE'] ?: $_SERVER['HTTP_CF_IPCOUNTRY']);
}
...
function var_export_modern($var, $indent = '')
{
    switch (gettype($var)) {
        case 'string':
        ...
        case 'array':
            ...
            foreach ($var as $key => $value) {
                $r[] = "$indent    "
                    . ($indexed ? '' : var_export_modern($key) . ' => ')
                    . var_export_modern($value, "$indent    ");
            }
            return "[\n" . implode(",\n", $r) . "\n" . $indent . ']';
        case 'boolean':
...
function getTimestamp($timestamp = 'today', $utc = true)
{
    return \Carbon\Carbon::parse($timestamp, $utc ? 'UTC' : null)->timestamp;
}

function cacheSerialize($value)
{
    return gzencode(serialize($value), 5);
}

function cacheUnserialize($value)
{
    if (is_null($value)) {
        return false;
    }
...
function t($text, $params = [], $language = null): string
{
    return \Yii::t('app', $text, $params, $language);
}

function hEncode($content)
{
    return htmlspecialchars($content, ENT_QUOTES, 'UTF-8', false);
}


function rmnl($content)
{
    return preg_replace('#[\s\t\r]+#i', ' ', $content);
}

function array_search_substring($array, $substring)
{
    $result = [];
...
function array_sum_key($array, $key)
{
...
function hashedImageUrl($hash): string
{
    return 'https://stx.myfresh.app/' . $hash;
}
...
function nl2p($string, $lineBreaks = true, $checkParagraphsExists = false)
{
    if ($checkParagraphsExists && preg_match('#<p(?:[^>]+|)>#', $string)) {
        return $string;
    }
...
function extractDomainFromUrl($url): string
{
    ...
    return preg_replace('#^(www|api|feeds|feed|rss)\.#i', '', $host);
}

function convertToHashTag($string): string
{
    return mb_strtolower(preg_replace('#[^\p{L}\w]#iu', '', $string));
}
