...
namespace common\components\scrapers\sources\de\Tag24;
...
/**
 * Class Scraper
 * @package common\components\scrapers\sources\de\Tag24
 *
 * @Config (
 * timezone="Europe/Berlin", urls={
 * "https://www.tag24.de/sitemap/myfresh/welt"
 * })
 */
    ...
    public function parseArticlesList(string $url): PromiseInterface
    {
        return Coroutine::of(function () use ($url) {
            ...
            for ($i = $articles->count() - 1; $i >= 0; --$i) {
                $node = $articles->eq($i);
                try {
                    $title = html_entity_decode($node->filterXPath('//title')->text());
                    ...
                    if ($publicationDate > $lastAddedPublicationTime) {
                        ...
                        $body->add(new ArticleBodyNode('paragraph', html_entity_decode($node->filterXPath('//description')->text())));
