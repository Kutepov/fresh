...
class Client
{
     ...
     */
    public function push(?callable $callback = null): array
    {
        $responseCollection = [];
        ...
        while ($running > 0 && $execrun === CURLM_OK) {
            ...
            while ($done = curl_multi_info_read($mh)) {
                ...
                [$token, $internalId] = json_decode(curl_getinfo($handle, CURLINFO_PRIVATE));
                ...
                $response = new Response($statusCode, $headers, (string)$body, $token, $internalId);
    ...
    private function prepareHandle(Notification $notification)
    {
        ...
        curl_setopt($ch, CURLOPT_PRIVATE, json_encode([$notification->getDeviceToken(), $notification->getInternalId()]));
