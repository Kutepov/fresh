...
namespace common\components\guzzle;
...
class GuzzleRetryMiddleware
{
     ...
     */
    protected function shouldRetryHttpResponse(array $options, ResponseInterface $response, RequestInterface $request): bool
    {
        ...
        switch (true) {
            ...
            default:
                $condition = false;
                if (is_callable($options['should_retry_call'])) {
                    $condition = call_user_func_array(
                        $options['should_retry_call'],
                        [
                            &$request,
                            $response,
                            &$options,
                ...
                return $condition || in_array($response->getStatusCode(), $statuses, true);
        }
    }
     ...
     * @return Promise
     */
    protected function doRetry(RequestInterface $request, array $options, ResponseInterface $response = null, $reason = null): Promise
    {
